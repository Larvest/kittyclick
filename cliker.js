...
function food_clicked(ass) {
    if (clicker.upgrades[ass].cost <= clicker.food) {
        ...
        clicker.upgrades[ass].cost += Math.round(clicker.upgrades[ass].cost * 0.15); // ну это как бы ебать ваще я не ебу че делает, но это короче формула увеличивающая цену с каждой покупкой
        update_upgrades();
    }
}
...
function updatecount() { // не трогать желательно
    ...
    setInterval(() => {
        for (i in clicker.upgrades) {
            ...
            clicker.foodPerSecond += clicker.upgrades[i].amount * clicker.upgrades[i].fps;
        }
        document.querySelector("#food").innerHTML = "У вас " + String(clicker.food).split(".")[0] + " корма"
        document.querySelector("#foodPerSecond").innerHTML = "У вас " + String(clicker.foodPerSecond).split(".")[0] + " корма в секунду"
        clicker.foodPerSecond = 0;
